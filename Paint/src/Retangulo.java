...
public class Retangulo extends Figura
{
	...
	public Retangulo(int x, int y, int l, int a, Color b, Color i)
	{
		super(b,i);
	...
	public boolean contains(int x, int y)
	{
		int base, altura;
		//1º e 4º quadrante:
		if(x >= this.getCentro().getX()){
			base = (int)Math.sqrt(Math.pow((x-this.getCentro().getX()),2));
			if(y <= this.getCentro().getY()){
				altura = (int)Math.sqrt(Math.pow((y-this.getCentro().getY()),2));					
			}else{
				altura = (int)Math.sqrt(Math.pow((this.getCentro().getY()-y),2));
			}
		}//2º e 3º quadrante:
		else{
			base = (int)Math.sqrt(Math.pow((this.getCentro().getX()-x),2));
			if(y >= this.getCentro().getY()){
				altura = (int)Math.sqrt(Math.pow((y-this.getCentro().getY()),2));					
			}else{
				altura = (int)Math.sqrt(Math.pow((this.getCentro().getY()-y),2));
